...
pub trait HighLevelPlanner {
    ...
    fn set_map(&mut self, map: Arc<dyn Map>);
}

pub trait LocalPlanner {
    fn get_desired_velocity(&self, agent: &Agent, recommended_velocity: Vec2f, map: Arc<dyn Map>) -> Vec2f;
}
...
pub struct Simulation {
    ...
    local_planner: HashMap<AgentId, Arc<dyn LocalPlanner>>,
    sim_time: std::time::Duration
}

impl Simulation {

    fn new(map: Arc<dyn Map>) -> Self
    {
        Self {
            agents: vec!(),
            map: map,
    ...
    fn add_agents(
        ...
        local_planner: Arc<dyn LocalPlanner>) -> Vec<AgentId>
    {
